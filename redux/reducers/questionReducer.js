...
function createQuestion() {
    ...
    const choices = getShuffledArray([
        firstParameter.toString() + secondParameter.toString(),
        secondParameter.toString() + firstParameter.toString(),
        answer
    ]);

    return {question: questionString, correctAnswer: answer, choices: choices}
}
...
const getShuffledArray = arr => {
    ...
    return [
        arr[rand], ...getShuffledArray(arr.filter((_, i) => i !== rand))
    ];
};